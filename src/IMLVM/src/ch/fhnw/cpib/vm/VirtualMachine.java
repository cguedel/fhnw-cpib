...
public class VirtualMachine implements IVirtualMachine {
    ...
    public class LoadImRatioExec extends LoadImRatio implements IExecInstr {
    	public LoadImRatioExec(Ratio value) { super(value); }
    	
    	public void execute() throws ExecutionError
    	{
    		if (sp > hp) { throw new ExecutionError(SP_OVER_HP); }
    		store[sp]= Data.ratioNew(value);
    		sp= sp + 1;
    		pc= pc + 1;
    	}
    }
    ...
    public class NumRatioExec extends NumRatio implements IExecInstr {
    	public void execute()
    	{
    		store[sp-1]= Data.ratioNum(store[sp-1]);
    		pc= pc + 1;
    	}
    }
    
    public class DenomRatioExec extends NumRatio implements IExecInstr {
    	public void execute()
    	{
    		store[sp-1]= Data.ratioDenom(store[sp-1]);
    		pc= pc + 1;
    	}
    }
    
    public class FloorRatioExec extends FloorRatio implements IExecInstr {
		public void execute() throws ExecutionError {
			store[sp-1] = Data.ratioFloor(store[sp-1]);
			pc= pc + 1;
		}
    }
    
    public class CeilRatioExec extends CeilRatio implements IExecInstr {
		public void execute() throws ExecutionError {
			store[sp-1] = Data.ratioCeil(store[sp-1]);
			pc= pc + 1;
		}
    }
    
    public class RoundRatioExec extends RoundRatio implements IExecInstr {
		public void execute() throws ExecutionError {
			store[sp-1] = Data.ratioRound(store[sp-1]);
			pc= pc + 1;
		}
    }
    ...
    public class InputRatioExec extends InputRatio implements IExecInstr {
        public InputRatioExec(String indicator) { super(indicator); }

        public void execute() throws ExecutionError
        {
            System.out.print("? " + indicator + " : ratio = ");
            Ratio input= InputUtility.readRatio();
            int address= Data.intGet(store[sp - 1]);
            store[address]= Data.ratioNew(input);
            sp= sp - 1;
            pc= pc + 1;
        }
    }
    ...
    public class OutputRatioExec extends OutputRatio implements IExecInstr {
    	public OutputRatioExec(String indicator) { super(indicator); }
    	
    	public void execute()
    	{
    		sp= sp - 1;
    		Ratio output= Data.ratioGet(store[sp]);
    		System.out.println("! " + indicator + " : ratio = " + output);
    		pc= pc + 1;
    	}
    }
}
